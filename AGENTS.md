...
- **외부 API 를 30건 이상 호출하지 않는다** — 시작 전에 대상·예상 호출 수·쿼터 영향을 보고하고 승인을 받는다.
  실검증도 예외가 아니며, 승인 전에는 1건으로만 확인한다.

  상세는 `backend/.claude/skills/kotlin-client` 의 "대량 호출은 시작 전에 승인받는다" 절.
  Claude 세션에서는 `.claude/hooks/warn-bulk-api-calls.sh` 훅이 확인 창을 띄우지만, 감지는 보조 수단일 뿐이다.
