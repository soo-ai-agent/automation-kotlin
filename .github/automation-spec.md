...
- 로컬 모델로도 이 저장소 작업을 할 수 있다. 그때도 **같은 코딩 규칙**이 적용된다.

### 구현

- Hermes 는 저장소 스킬을 `./.agents/skills` 에서 읽는다 (`hermes skills trust` 이후).
  그 폴더에 우리 스킬 31종을 가리키는 심볼릭 링크를 커밋해 둔다 — clone 하면 바로 붙는다.

- 모델·엔드포인트는 Hermes 설정이 정한다 (`hermes model`). 저장소는 관여하지 않는다.

### 수정 시 불변 조건

- **스킬을 늘리면 `.agents/skills/` 링크도 늘린다.** 링크가 없으면 Hermes 세션은 그 규칙을 못 본다 —
  `static.sh` 가 세 스킬 폴더를 훑어 빠진 링크를 잡는다.

- `speckit-*` 링크는 커밋하지 않는다. 가리키는 대상이 설치물이라 저장소에 없다.

- **영역 분리는 Hermes 에 없다.** Claude Code 는 `--add-dir` 로 영역을 가르지만 Hermes 는 저장소 스킬을 통째로 읽는다.
  사람이 직접 쓰는 세션이라 그 전제가 성립하고, 자동 노드는 여전히 Claude Code 쪽에서 돈다.

## 4.5 로컬 오케스트레이션

### 요구사항

- 사람이 자기 컴퓨터에서 계획 하나를 끝까지 돌릴 수 있다. GitHub 을 거치지 않는다.

- 역할·도구 제한은 CI 와 **같은 파일**을 쓴다. 두 벌이 되면 어느 쪽이 진짜인지 알 수 없다.

### 구현

- `.claude/workflows/work.js` — Claude Code Workflow 도구가 읽는 스크립트. 계획 표현식을 단계로 나누고
  단계마다 역할을 `agentType` 으로 부른다. 한 단계 안은 `parallel`, 단계 사이는 순서를 지킨다.

- 역할 원본은 `.github/agent/nodes/` 다. `bin/claude-skills.sh` 가 `.claude/agents/` 로 복사하고,
  `.gitignore` 가 그 사본을 무시한다.

### 수정 시 불변 조건

- **`.claude/agents/` 를 커밋하지 말 것.** 커밋되는 순간 작업 브랜치가 자기 역할 파일을 고쳐 권한을 넓힐 수 있다.
  `static.sh` 가 gitignore 에 있는지와 커밋되지 않았는지를 본다.

- 세션 하나가 상태를 들고 있으므로 `state.sh`·`plan-stage.sh`·`next-role.sh` 를 쓰지 않는다.
  **그것들을 여기에 끌어오지 말 것** — 그 셋은 프로세스가 죽는 CI 를 위한 것이다.

- 무인 실행은 여전히 Actions 쪽이다. 로컬 워크플로는 **컴퓨터를 켜 둔 동안만** 돈다.
