...
//
// 출력: stages=<단계 배열의 JSON>. 각 단계는 그 단계에서 나란히 돌 노드들이다.
//   각 항목: { node, label, rescue, make_pr }
//
// 단계 수에 상한이 없다. 워크플로가 단계마다 잡을 미리 선언하지 않고, 잡 하나가
// 이번 단계를 돌린 뒤 자기 자신을 다음 단계로 다시 부르기 때문이다.
//
// GitLab 판과 다른 점 — 수습(?)은 별도 잡이 아니라 노드 잡 안에서 처리한다.
// 같은 브랜치를 이어받아 완수한다는 의미는 같은 잡 안에서 이어 돌리는 편이 더 정확하고,
// 아티팩트로 상태를 주고받을 필요도 없다.
...
const lines = [`stages=${JSON.stringify(out)}`];
