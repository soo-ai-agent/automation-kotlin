...
# (tests/cases.sh — 케이스는 cases/next-role/).
#
# 받는 값 (환경변수):
...
# ④ 노드가 너무 많이 돌았다. 계획이든 재작업이든 루프 전체의 상한이다.
#    ⑦ 의 라운드 상한과 다르다 — 저쪽은 같은 PR 을 몇 번 다시 보는지다.
...
#
#    **리뷰 전용 상한(⑦ 라운드)보다 앞에 둔다.** 계획 중에는 리뷰가 돈 적이 없어 라운드가
#    언제나 0 인데, 상한도 넘어오지 않아 0 이라 `0 >= 0` 으로 첫 걸음에서 막힌다.
#    아래로 내려가는 것은 판정이 있는 호출뿐이므로, 여기부터는 리뷰 맥락이다.
if [ -z "$VERDICT" ]; then
...
# ⑦ 같은 PR 을 너무 많이 봤다. 여기부터는 리뷰 맥락이다 (⑥ 에서 계획이 걸러졌다).
