...
#
# 상태는 **이슈·PR 코멘트 안의 숨은 블록**에 산다. 다른 후보를 고르지 않은 이유:
#   저장소 커밋 — `.claude-split/`·`.claude-spec.md` 를 커밋하지 않는 규약과 어긋나고, PR diff 를 더럽힌다.
#   Actions 아티팩트 — 실행 하나에 묶인다. 루프가 자기를 재호출하면 새 실행이라 이어지지 않는다.
#   이슈 라벨 — 값이 아니라 표시라서 숫자를 담기에 맞지 않는다.
# 코멘트는 PR 이 열리기 전(이슈 단계)부터 있고, 실행이 바뀌어도 남고, 사람 눈에는 안 보인다.
#
...
#   step         루프가 지금까지 돈 걸음 수 — 노드가 올린다. 무한 루프 브레이크의 근거다
#   last_role    마지막에 돈 역할
...
# 블록이 아예 없으면 첫 바퀴다 — 기본값에 broken=0.
