...
#   # expect: done | human | loop-off | <역할 이름>
...
pick() { # $1=판정 $2=라운드 $3=라운드상한 $4=PAT $5=실행횟수 $6=실행상한 $7=상태읽기실패
    VERDICT="$1" ROUNDS="$2" MAX_ROUNDS="$3" HAS_PAT="$4" \
        STEP="$5" MAX_STEPS="$6" BROKEN="$7" bash "$NEXT" 2>/dev/null
}
...
for case_file in "$CASE_DIR"/*.case; do
    ...
    got=$(pick "$(head_of verdict)" "$(head_of rounds)" "$(head_of max-rounds)" "$(head_of has-pat)" \
               "$(head_of step)" "$(head_of max-steps)" "$(head_of broken)")
