...
- **검사는 다섯으로 나눈다** — `static.sh`(형식) · `graph.sh`(그래프 펼치기) · `loop.sh`(루프 판단) · `state.sh`(루프 상태)는
  모델을 안 불러 공짜고, `run.sh` 만 케이스당 claude 호출 1건이다. CI 는 싼 것을 먼저 돌려 걸러낸다.

- **루프 상태는 이슈·PR 코멘트 안의 숨은 블록(`<!-- claude-state ... -->`)에 산다.** 노드가 걸음 수와 마지막 역할·결과를,
  ...
  **블록이 아예 없는 것(첫 바퀴)과 있는데 못 읽는 것(`broken=1`)은 다르다.** 섞으면 처음과 고장을 구분할 수 없다.
