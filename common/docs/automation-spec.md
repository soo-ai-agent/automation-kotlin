...
| 하네스 루프 회귀 | `common/harness-tests/loop.sh` | 사람이 손으로만 실행 (비용이 커서 CI 에 넣지 않음) |
...
- **검사는 셋으로 나눈다.** 싼 것부터 돌린다.

  | 검사 | 보는 것 | 비용 | 도는 곳 |
  |---|---|---|---|
  | `static.sh` | 규칙이 읽히기는 하는가 (링크·경로·케이스 형식) | 모델 호출 0건 | CI, 판정 회귀보다 먼저 |
  | `run.sh` | 잡아야 할 위반을 잡는가 | 케이스당 1건 | CI |
  | `loop.sh` | 지적을 따라 고치면 통과에 닿는가 | 케이스당 최대 (1 + 라운드수 × 2)건 | 사람이 손으로 |

  링크가 죽거나 경로가 어긋나면 규칙은 파일에 남아 있어도 아무도 안 읽는다. 이건 모델을 안 불러도 잡히므로 `static.sh` 가 먼저 본다.

- **`loop.sh` 의 리뷰어와 작성자는 서로 다른 claude 세션이다.** 한 세션이 자기가 쓴 코드를 자기가 심사하면
  통과 판정이 쉬워져 검사가 되지 않는다. 역할 지시문도 각자 실제 워크플로가 쓰는 파일에서 온다 —
  리뷰어는 `review-role.md`, 작성자는 `nodes/fix.md` 다.

  작성자에게는 역할 지시문 뒤에 하네스 입출력 규약(파일을 고치지 말고 새 diff 만 출력)을 덧붙인다.
  하네스에는 실제 파일이 없기 때문이며, 역할 자체를 바꾸는 것이 아니라 결과를 받는 방식만 정하는 것이다.

- **`loop.sh` 는 예상 호출이 30건 이상이면 `--yes` 없이 돌지 않는다.** CLAUDE.md 의 대량 호출 승인 규칙을
  스크립트가 직접 지키게 한 것이다. 줄이려면 `--rounds` 를 낮추거나 영역을 좁힌다.

- **차단 기대 케이스는 루프 대상 여부를 파일 머리에 밝힌다** — `# loop: converge` 또는 `# loop: skip — <이유>` 다.
  `loop.sh` 는 `converge` 인 것만 돌리고, `static.sh` 가 둘 중 하나가 반드시 있는지 검사한다.

  고치는 데 배포가 여러 번 필요한 변경은 diff 하나로 끝낼 수 없어 라운드를 늘려도 수렴하지 않는다.
  그런 케이스를 빼는 것은 정당하지만, 이유 없이 빠지면 "수렴 못 해서 슬쩍 뺀 것"과 구별되지 않으므로 이유를 함께 적는다.
