...
| 층 | 무엇 | 어디 |
|---|---|---|
| **프레임워크 규칙** | 폴더 구조·라우팅·스타일·데이터 페칭 — Expo 공식 문서 그대로 | [`expo/`](expo/README.md) (벤더링, 17종) |
...
`expo/` 는 한 단계 깊어서 **자동으로 읽히지 않는다** — 아래 스킬이 가리키는 자리에서 직접 연다.
...
`lint` 는 포맷만이 아니라 React 훅·ref 규칙도 본다. `e2e` 를 `build` 보다 먼저 돌리는 것은 개발 서버가
