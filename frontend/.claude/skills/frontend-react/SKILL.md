...
- 예외는 하나뿐: **자기 표현 상태** — ① 밖으로 안 나가고 ② DOM·타이머처럼 그 자리에서만 알 수 있고 ③ 서버 데이터도 업무 규칙도 아닌 값. 셋을 다 만족해야 하고, 애매하면 훅이다. (1-1장)
...
- MUST: 한 줄 100자 자제. **import 는 120자까지 한 줄**로 쓰고, 넘으면 3줄 래핑(지정자를 몰아 적은 한 줄). 여러 줄 객체·return·구조분해도 같은 채워 적기 — 100자 근처까지 채우고 넘치면 다음 줄. 세로 나열은 항목별 주석이 있을 때만. (0-3장)
...
- MUST: **DTO 의 nullable 은 서버 코드를 열어 정한다.** 추측으로 붙인 `?` 는 계약을 둘로 쪼갠다 — 서버 non-null이면 필수, nullable이나 키는 오면 `| null`, null이면 키가 빠지면(`@JsonInclude(NON_NULL)`) `?`. "누락 시 0" 류 주석은 위험 신호다. (11-4장)

- MUST: **매퍼가 값을 보장하면 타입도 보장한다.** 매퍼가 늘 숫자를 만드는데 타입이 `?: number` 면 화면이 평생 헛분기한다. (11-4장)

- MUST: 프론트가 부르는 **엔드포인트 목록과 서버 매핑 목록의 차집합이 0**이어야 한다. 없는 경로를 발견하면 폴백을 넣지 말고 보고한다. (11-5장)

- MUST: `x?: T \| null` 은 두 없음이 다른 뜻일 때만 쓴다 — 같은 뜻이면 하나로 모은다. 판정: "안 넘김"과 "null"이 서로 다른 일을 시키는가. (11장)
...
| 서버 코드를 안 보고 붙인 DTO `?` ("누락 시 0" 주석) | 계약이 둘로 쪼개진다 (11-4장) | **Critical** |
| 서버에 없는 엔드포인트를 부르는 화면 | 영원히 404 (11-5장) | **Critical** |
...
- [ ] 컴포넌트에 남은 상태가 1-1장 예외 셋을 전부 만족하는가
