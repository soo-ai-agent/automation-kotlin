...
**예외는 와이어 DTO 하나다.** 서버가 평평하게 보내는 응답은 묶으면 모양이 거짓이 된다. 이때만 사유를 타입 위에 한 번 적되, **"이 필드들은 함께 있거나 함께 없다"는 사실과 그 근거(서버 코드 어디를 봤는지)를 함께 적는다** — 그게 필드별로 흩어 적을 때 잃는 정보다.
...
### 11-4. DTO 의 nullable 은 **서버 코드를 열어** 정한다 (MUST)

프론트 DTO 는 서버 계약의 사본이다. **추측으로 `?` 를 붙이면 그 순간 계약이 두 개가 된다** — 서버는 늘 보내는데 화면은 "안 올 수도 있다"고 분기하고, 매퍼는 이미 값을 채우고 있는데 타입만 없을 수 있다고 말한다. 이러면 데이터가 어디서 비는지 아무도 모른다.

**규칙: DTO 필드에 `?` 나 `| null` 을 쓰기 전에 서버의 응답 타입을 연다.** "아마 없을 수도"는 근거가 아니다.

| 서버가 이렇게 주면 | 프론트는 이렇게 |
|---|---|
| non-null 필드 | **필수**. `?` 도 `\| null` 도 붙이지 않는다 |
| nullable 인데 키는 늘 보냄(직렬화 옵션 없음) | `\| null` 만 |
| nullable 이고 **null 이면 키를 생략**(`@JsonInclude(NON_NULL)` 등) | `?` 만 |
| 하위호환으로 **정말** 없을 수 있는 신규 필드 | `?` + "언제부터 생긴 필드인지" 주석. 서버에 이미 있으면 이건 해당 없다 |

읽는 법 — Kotlin/Spring 기준:

- 필드 타입 끝의 `?` 가 nullable 여부다(`val score: Int?`).
- 클래스 위 `@JsonInclude(JsonInclude.Include.NON_NULL)` 이 있으면 **null 인 필드는 키 자체가 빠진다** → 그 필드만 `?`.
- 없으면 nullable 이어도 `"field": null` 로 실려 온다 → `| null`.

**"누락 시 0" 류 주석은 위험 신호다.** 서버가 늘 보내는 값에 그 주석이 붙어 있으면 십중팔구 추측으로 붙인 `?` 다 — 서버를 열어 확인하고, 필수로 바꾼다.

**매퍼가 값을 보장하면 타입도 보장해야 한다.** `p95DurationMs: num(r.p95DurationMs)` 처럼 매퍼가 항상 숫자를 만드는데 타입이 `?: number` 면, 화면은 오지 않을 `undefined` 를 평생 분기한다.

바로잡으면 **테스트 픽스처가 먼저 깨진다.** 실제 서버가 보내지 않는 모양을 흉내내고 있었다는 뜻이니, 픽스처를 실 응답에 맞춘다 — 타입을 되돌리지 않는다.
...
프론트: api/services 의 URL 상수를 모아 목록으로   →  차집합이 0 이어야 한다
```
