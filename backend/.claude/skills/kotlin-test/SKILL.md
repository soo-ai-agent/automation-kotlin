...
**자리:** 대상과 같은 패키지의 `src/test/kotlin/...`, 파일명은 `<대상>Test.kt`
**실행:** `./gradlew unitTest` — 태그가 없는 테스트만 돈다. DB·스프링 컨텍스트가 필요 없어야 한다.

## 요구사항이 오면 테스트도 함께 온다

새 동작(분기·정책·검증·상태 변경)을 만들면 **같은 변경에 유닛 테스트를 포함한다.**
"나중에 테스트 추가"는 없다. 테스트가 없는 새 동작은 미완성이다.
