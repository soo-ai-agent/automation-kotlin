...
**(5) 처음 보는 사람 리허설 — diff 와 문서를 남의 눈으로 다시 읽는다.**

내가 쓴 코드는 나에게만 자명하다. 커밋 직전에 변경 전체(코드 + 그 변경이 건드린 문서·주석)를
처음 보는 사람으로 한 번 더 읽고, 아래 신호를 찾는다. 전부 실제로 놓쳤던 것들이다.

| 신호 | 조치 |
|---|---|
| 같은 로직·상수가 두 파일에 | 소유자를 찾아 한곳으로 — 파생 값이면 그 값을 아는 도메인에게 |
| **이름만 다른 복제** — 이름 충돌을 피하려 개명했다면 | 그 개명이 공용화하라는 신호다. 복제 말고 공유 |
| 코드는 바뀌었는데 문서·주석·예시가 옛말을 한다 | 바꾼 낱말로 전수 검색해 함께 고친다 |
| 문서의 예시 값이 실측이 아니라 추정이다 | 실행해서 얻은 값만 적는다 — 라벨 하나도 틀린다 |

**숫자를 내는 검증은 그 숫자의 단위를 먼저 확인한다.** 예상과 반대되는 결과는 반드시 조사받지만
예상과 같은 결과는 조사받지 않는다 — 그래서 틀린 도구가 우연히 맞는 답을 내면 그 도구를 믿게 된다.

집계가 특히 그렇다. `git status --porcelain --ignored` 는 무시된 **디렉터리**를 한 줄로 찍는다.
이 저장소에서 재 보면 `grep -c` 로 24, `--untracked-files=all` 을 붙이면 42955 다.
세는 대상이 파일인지 줄인지 디렉터리인지 확인하고, 열거가 싸면 세지 말고 열거해서 읽는다.

보고에는 **하지 못한 검증**도 적는다. 빠뜨린 것을 적지 않으면 다음 사람이 속는다.

## 동치 증명 — 리팩토링이 동작을 바꾸지 않았음을 보인다

구조만 바꿨다고 말하려면 **바꾸기 전과 후가 같다는 증거**가 있어야 한다. 테스트 통과는
"내가 쓴 테스트가 덮는 범위 안에서 같다"는 말일 뿐이다. 셋을 대조한다.

| 재는 것 | 방법 | 같아야 하는 것 |
|---|---|---|
| 테스트 실계수 | 위 (1) | 이전 계수와 동일 |
| 엔드포인트 목록 | 기동 로그 또는 `actuator/mappings` 를 전후로 저장해 `diff` | 매핑 수·경로 전부 동일 |
| 응답 본문 | 같은 요청을 전후로 실호출해 `diff` | 필드명·값·상태코드 동일 |

```bash
# 바꾸기 전에 찍어 둔다
curl -s localhost:<포트>/actuator/mappings | python3 -m json.tool > /tmp/before.json
curl -s "localhost:<포트>/api/v1/<자원>?..."                        > /tmp/before-body.json
# 바꾼 뒤 같은 명령으로 after 를 만들고 diff 한다
```

**401·404 는 증명이 되지 않는다.** 매핑이 사라져도 같은 상태코드가 나오기 때문이다.
인증이 걸린 경로는 실제 토큰으로 200 을 받아 대조한다.

이 대조를 못 했으면 커밋 본문에 **"동치 증명 못 함"** 이라고 적는다.
