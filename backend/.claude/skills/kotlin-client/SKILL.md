...
**외부 API 를 100건 이상 호출하는 작업은 시작 전에 사람에게 보고하고 승인을 받는다.**
...
건수가 100 미만이어도 **유료 호출이거나 비멱등(결제·발송)이면** 같은 절차를 밟는다.
...
| 승인 없이 외부 API 를 100건 이상 호출 (실검증 포함) | 되돌릴 수 없는 쿼터·비용 소진 | Critical |
...
- [ ] 100건 이상 호출할 일이 있었다면 대상·예상 호출 수·쿼터 영향을 보고하고 승인받았는가
