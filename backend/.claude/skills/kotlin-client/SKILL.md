...
## 예외 — 관측 채널(알림 웹훅)은 모니터링 모듈이 소유한다

알림기의 본체는 스로틀·뮤트·이력(운영 정책)이고 HTTP 발송은 그 끝단이다. 떼어 client
모듈을 만들면 소비자가 모니터링 하나뿐이라 "바깥 import 수" 기준(kotlin-module-layout)에
미달한다. 대신 통신 규격(웹훅 페이로드)이 그 모듈 밖으로 새지 않는 것은 똑같이 지킨다.
