...
**애그리게이트는 "항상 함께 바뀌어야 하는 엔티티 묶음"이다.** 주문과 주문항목처럼, 따로 바꾸면 앞뒤가 안 맞는 것들을 하나로 묶는다.

그 묶음의 대표를 **루트**라고 하고, 바깥은 루트만 안다. 주문항목을 바꾸고 싶으면 주문에게 부탁한다 — 그래야 "금액 합계가 항목과 맞는다" 같은 규칙을 주문이 지킬 수 있다.
