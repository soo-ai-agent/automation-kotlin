...
| (라벨 없음) | 사람끼리 논의할 이슈, **에이전트에게 맡기지 않기로 한 영역** | — | 에이전트가 건드리지 않는다 |
...
**회원 인증(로그인·회원가입·토큰·비밀번호·권한 설정)은 사람이 직접 만든다.** 이 영역의 이슈에는 라벨을 붙이지 않는다.

디스패처는 `claude`·`claude-split` 라벨이 붙은 이슈만 집어가므로, **라벨을 안 붙이는 것만으로 충분하다.** 별도 설정은 없다.

주의할 것은 **간접 접촉**이다. "할 일 목록에 내 것만 보이게" 같은 이슈는 인증 기능이 아니지만 소유자 스코프 쿼리를 건드린다.

이런 이슈는 맡겨도 된다 — 규칙(`backend/.claude/skills/kotlin-auth`)이 살아 있어 리뷰어가 소유자 조건 누락이나 403·404 유출을 잡는다.

다만 **인증 기반 자체를 바꾸는 일**(`SecurityConfig`, 로그인 흐름, 토큰 발급·검증)은 이슈 본문의 "하지 말 것"에 적어 두거나 아예 맡기지 않는다.
...
- 목록 화면: frontend/src/screens/todo/index.tsx
```

- 파일 경로를 알면 적어라 — 에이전트가 찾는 시간이 줄고 정확도가 오른다.

- **새 기능이면 스펙 경로를 적어라** (`specs/003-주문취소/spec.md`). 에이전트가 그 문서를 기준으로 삼는다 ([스펙 쓰는 법](sdd-guide.md)). 버그 수정처럼 합의할 것이 없으면 스펙 없이 이 이슈만으로 간다.
