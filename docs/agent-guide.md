...
리뷰어가 머지를 막는 것은 세 가지뿐이다 — [코드 리뷰 규칙](../rules/code-review.md)의 MUST 위반, 해당 영역 스킬의 Critical 항목, 그리고 이슈가 스펙(`specs/`)을 가리킬 때 스펙과 어긋난 구현. 스타일 취향이나 있으면 좋을 개선은 통과시키고 코멘트로만 남긴다.
...
- **순차 단계(`>`) 수에 제한이 없다.** 잡 하나가 이번 단계를 돌린 뒤 자기를 다시 부르기 때문이다. 병렬(`+`)로 묶는 수에도 제한이 없다.

- 대신 노드가 통틀어 도는 횟수에 상한이 있다 — `CLAUDE_MAX_STEPS`(기본 12). 닿으면 사람을 부른다.
...
| 변수 | 기본값 | 바꾸면 |
|---|---|---|
| `CLAUDE_GRAPH` | `code` | 노드 구성 (`>` 순차 · `+` 병렬 · `?` 수습). 순차 단계 수에 상한 없음 |
...
스펙 먼저 쓰기 쪽에도 사람이 관리하는 설정이 하나 있다 — `.specify/preset-ko/speckit-version.txt` 가 이 프로젝트의 spec-kit 버전을 고정한다. 노드는 이 값을 읽지 않는다.
