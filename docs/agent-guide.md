...
선택 기능인 스펙 먼저 쓰기를 켜면 맨 앞이 **스펙 작성 → 이슈 작성**으로 한 단계 늘어난다. 아래 그림은 그대로다 — 스펙은 CI 밖에서 만들어지고, 노드가 하는 일은 바뀌지 않는다 ([아래 절](#스펙-먼저-쓰기는-ci-밖에서-돈다)).
...
| 스펙 작성 (선택 기능을 켰다면) | **사람** — 아래 참고 |

## 스펙 먼저 쓰기는 CI 밖에서 돈다

선택 기능인 스펙 먼저 쓰기([sdd-guide.md](sdd-guide.md))를 켜도 **노드는 그 도구를 쓰지 않는다.**
