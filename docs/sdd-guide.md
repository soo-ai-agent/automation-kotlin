...
**이 저장소는 스펙을 먼저 쓴다.** 만들 것을 스펙 문서로 고정하고, 그 스펙을 근거로 구현한다. [spec-kit](https://github.com/github/spec-kit) 을 얹어서 쓴다.

요구사항을 글로 먼저 고정하고 그 글을 고쳐 가며 합의하는 방식이라, 합의 비용을 앞으로 당긴다. "만들고 보니 원하던 게 아니었다" 를 구현 전에 잡는 것이 목적이다.

## 스펙을 건너뛰어도 되는 때

**버그 수정과 작은 변경은 이슈 한 줄로 간다.** 오타, 문구 수정, 한 줄 로직 고치기처럼 요구사항을 합의할 것이 없는 일에는 단계가 늘어난 만큼 시간과 토큰만 더 든다.

기준은 하나다 — **"무엇을 만들지"에 이견이 생길 수 있는가.** 이견이 없으면 이슈로, 있으면 스펙으로 간다.

이슈로 갈 때의 절차는 [issue-guide.md](issue-guide.md) 에 있다.
...
- `/speckit-checklist` — 요구사항 품질 검토 체크리스트를 만든다
- `/speckit-converge` — 이미 짜인 코드와 스펙을 대조해 남은 작업을 `tasks.md` 에 덧붙인다
- `/speckit-taskstoissues` — 작업 목록을 GitHub 이슈로 바꾼다
...
| 문서 | 언제 쓰나 |
|---|---|
| `TASK.md` | 맨 처음 저장소를 구축할 때 한 번 |
| `specs/` | 그 뒤에 기능을 추가할 때 — **기본 경로다** |
| 이슈 본문 | 버그 수정과 작은 변경 (위 "건너뛰어도 되는 때") |
| `CONTRACT.md` | API 계약 — `api` 노드가 쓰고 `web` 노드가 읽는다. 스펙과 별개로 그대로다 |
...
리뷰어도 같은 기준을 지킨다 — 이슈가 가리키는 스펙을 찾아 구현과 대조하고, 어긋나면 머지를 막는다.
