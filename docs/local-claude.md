...
| 스킬 이름·설명 — 공통 8종 + 그 영역(백엔드 17 · 프론트 7) | 명령 즉시 — launcher 가 그 영역을 `--add-dir` 로 열어 준다 |
...
마지막 줄만 즉시가 아니다. 스킬이 32종이라 본문을 전부 미리 읽으면 한 번에 다룰 수 있는 분량을 넘긴다.

하위 폴더(`backend/`·`frontend/`)의 스킬은 `--add-dir` 로 열어 줘야 목록에 뜬다. 저장소 루트에서 그냥 `claude` 를 치면 공통 8종만 보인다.
