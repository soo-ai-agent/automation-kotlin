...
**그냥 `claude` 로 열면 세션이 시작될 때 공통 6종만 붙는다.** claude 는 연 자리의 `.claude/skills/` 부터 뒤지므로
...
이 저장소의 코딩 규칙은 Hermes 에서도 그대로 적용된다. `.agents/skills/` 가 우리 스킬 31종을 가리키고 있고,
...
| 스킬의 이름·설명 (영역에 따라 23·14·31종) | 명령 즉시 — launcher 가 `--add-dir` 로 하위 스킬 폴더를 붙인다 |
...
마지막 줄만 즉시가 아니다. 스킬이 31종이라 본문을 전부 미리 읽으면 한 번에 다룰 수 있는 분량을 넘긴다.
