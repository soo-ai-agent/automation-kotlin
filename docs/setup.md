...
준비물은 토큰 두 개, 실행할 것은 스크립트 둘이다 — 자동화를 켜는 것과 스펙 먼저 쓰기를 켜는 것.
...
| 프론트 설정 | 기본값으로 바로 뜬다. 주소·키는 `frontend/app.json` 의 `extra` 한 곳에 있다 ([frontend/README.md](../frontend/README.md)) |
| 지도·광고를 쓸 때 | 그때만 키를 채운다 — 지도는 `kakaoJsKey`([frontend/docs/map.md](../frontend/docs/map.md)), 광고는 `admob*`([frontend/docs/ads.md](../frontend/docs/ads.md)·[ads.md](ads.md)). 비워 두면 그 모듈만 안 뜨고 나머지는 정상이다 |
